...
import { Student , Agent,User,Admin} from '../model';
...
export class ApiService {
  ...
  getListAgent(): Observable<Agent> {
    return this.http
      .get<Agent>(this.base_path + '/agents/' + 'list')
      .pipe(
        retry(2),
        catchError(this.handleError)
      )
  ...
  // Get clients data
  getListClient(): Observable<User> {
    return this.http
      .get<User>(this.base_path + '/clients/' + 'list')
      .pipe(
        retry(2),
        catchError(this.handleError)
      )
  ...
  getListAdmin(): Observable<Admin> {
    return this.http
      .get<Admin>(this.base_path + '/admin/' + 'list')
      .pipe(
        retry(2),
        catchError(this.handleError)
      )
